        ...
        try:
            if self.path.endswith("/"):
                ...
                for startup in startups:
                    output +="<br>"
                    output += "<a href = /startups/{}/show > {} </a> ".format(startup.id, startup.name)
                    output +="<br>"
            ...
            if self.path.endswith("/show"):
                ...
                if startup:
                    ...
                    for founder in founders:
                        output+="<li>{}: {} </li>".format(founder.name, founder.bio)
                    output += "</ul>"
                    output += "<br>"
                    output += "<form method ='POST' enctype='multipart/form-data' action ='/startups/%s/show'>"%startup.id
                    output += "<label> Founder Name</label>"
                    output += "<input type='text' name='name'/>"
                    output += "<label> Bio </label>"
                    output += "<input type='text' name='bio'/>"
        ...
        try:
            ...
            if self.path.endswith("/show"):
                ...
                if ctype == 'multipart/form-data':
                    ...
                    if startup:
                        print(fields['bio'][0])
                        founder = Founder(name=fields['name'][0], bio= fields['bio'][0], startup_id=startup_id)
                        session.add(founder)
                        session.commit()
                        print("!!!")
